...
// Returns { body: string|null, sources: Array<{name,url}>|null }
// dirDistrictId: the URL/directory key, e.g. "AK-01" or "CA-12"
// Scans rep-info/ for the first file whose name starts with "{state}-{num}-"
// so lookup is robust to name changes and middle-initial variations
export function loadRepBioWithSources(dirDistrictId) {
  const [state, num] = dirDistrictId.split("-");
  const prefix = `${state.toLowerCase()}-${num.toLowerCase()}-`;
  const dir = path.join(process.cwd(), "public", "content", "rep-info");
  try {
    const match = fs.readdirSync(dir).find((f) => f.startsWith(prefix) && f.endsWith(".md"));
    if (!match) return { body: null, sources: null };
